...
def test_post():
    ...
    # POST 메소드 - requset.form - ajax.data 세트로 작업함
...
def test_zz():
    title_receive = request.form['title_zz']
    print(title_receive)
    return jsonify({'result': 'success', 'msg': '이 요청은 POST + zz!'})


@app.route('/test', methods=['GET']) #GET 메소드 응답만 받겠다
def test_get():
    # request에는 요청에 대한 정보가 담겨있음
    # request.args -> 쿼리스트링 정보가 있음
    # 쿼리스트링 -> www.naver.com/?query=호랑이&name=홍길동
    # ? 기호 이후의 주소가 쿼리스트링
    # 아래의 정보가 requst.args에 저장됨 (딕셔너리)
    # query=호랑이&name=홍길동 -> {'query': '호랑이', 'name': '홍길동'} 
    # GET 메소드 - requset.args -쿼리스트링 세트로 작업함
...
def test_happy():
    title_receive = request.args.get('title_happy')
    print(title_receive)
    return jsonify({'result': 'success', 'msg': '이 요청은 GET + happy!'})
